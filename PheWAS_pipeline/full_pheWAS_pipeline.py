...
def compute_sex(pheno):
    ...
    # pheno['Sex_genetic'][pheno['Sex_genetic'].isna()] = pheno['Sex'][pheno['Sex_genetic'].isna()]
...
def run_pheWAS(sample_list, pheno_dict, out_dir, raw_dir, data_dir, reg):
    ...
    for file in sample_list:
        ...
        with open(data_dir + '/' +  file_prefix + '_fixed_cov_fields.txt', "r") as f:
        ...
        for geno_file in raw_geno_files:
            ...
            for ethnicity_group in ethnicity_groups:
                ...
                # remove the ethnicity_group that we want to retain (we will drop the rest of the columns)
                cols_to_remove.remove(ethnicity_group)
                ...
                # drop the f.eid column and the other ethnicities in the phenotype data since we no longer need them
