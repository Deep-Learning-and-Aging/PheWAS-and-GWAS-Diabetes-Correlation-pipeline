...
#SBATCH -t 0-8:00:00                                                                               # Runtime in D-HH:MM format
#SBATCH -p short                                                                                    # Partition to run in
#SBATCH --mem=128000M                                                                                # Memory total in MiB (for all cores)
#SBATCH -o /home/tad368/PheWAS-and-GWAS-Diabetes-Correlation-pipeline/batch_logs/full_pheWAS_pipeline%j.out   # File to which STDOUT will be written, including job ID (%j)
#SBATCH -e /home/tad368/PheWAS-and-GWAS-Diabetes-Correlation-pipeline/batch_logs/full_pheWAS_pipeline%j.err   # File to which STDERR will be written, including job ID (%j)
...
python3 ~/PheWAS-and-GWAS-Diabetes-Correlation-pipeline/python_scripts/full_pheWAS_pipeline.py \
                                ...
                                --variants ~/PheWAS-and-GWAS-Diabetes-Correlation-pipeline/variants_list/Abdomen_Top_SNPS.tsv \
                                --samples ~/PheWAS-and-GWAS-Diabetes-Correlation-pipeline/in_pop_IDs/Liver_in_sample_pop_IDs.txt \
                                --variants ~/PheWAS-and-GWAS-Diabetes-Correlation-pipeline/variants_list/Liver_Top_SNPS.tsv \
                                --samples ~/PheWAS-and-GWAS-Diabetes-Correlation-pipeline/in_pop_IDs/Pancreas_in_sample_pop_IDs.txt \
                                --variants ~/PheWAS-and-GWAS-Diabetes-Correlation-pipeline/variants_list/Pancreas_Top_SNPS.tsv \
                                --threads 16 \
                                --memory 128000 \
                                --location ~/data_dir \
